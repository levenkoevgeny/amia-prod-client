...
export const productsGroupAPI = {
  async getItemsList(token, searchForm = { group_name: "" }) {
    let { group_name } = searchForm
    return axios.get(
      `${process.env.VUE_APP_BACKEND_PROTOCOL}://${process.env.VUE_APP_BACKEND_HOST}:${process.env.VUE_APP_BACKEND_PORT}/api/${base_url}/?group_name__icontains=${group_name}`,
      authHeaders(token)
    )
  },
